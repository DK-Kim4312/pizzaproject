...
import './Sidebar.css';
import { fetchData, fetchDataByProductKey, saveProductKey } from '../API.js';
...
const Sidebar = () => {
  ...
  const handleIngredientClick = (ingredient) => {
    ...
    // Get the product key for the clicked ingredient
    const productKey = linkedValuesByIndex[ingredient];
    
    // Get the ingredient name using the product key
    const ingredientName = findKeyByName(productKey);

    console.log(`Key for ${ingredientName}: ${productKey}`);
    saveProductKey(productKey);
    fetchDataByProductKey(productKey);
  };
